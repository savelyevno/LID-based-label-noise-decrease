...
if __name__ == '__main__':
    ...
    # noise_matrix = get_pair_confusion_noise_matrix(class_count, 0.45, True)
    noise_matrix = None

    model_name = '60_paper_ep50_init0_rst1-cut-none 2'
    ...
    model = Model(dataset_name=dataset_name, model_name=model_name,
                  ...
                  train_separate_ll=False, separate_ll_class_count=2, separate_ll_count=None, separate_ll_fc_width=64,
                  log_mask=1 * (1 << 0) +
                           0 * (1 << 1) +
                           0 * (1 << 2) +
                           0 * (1 << 3) +
                           ...
                           0 * (1 << 6) +
                           1 * (1 << 7))
    model.train(noise_ratio=0.6, noise_seed=0, noise_matrix=noise_matrix)
