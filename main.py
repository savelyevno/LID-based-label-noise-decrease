...
if __name__ == '__main__':
    ...
    model_name = '20_paper_ep100_init30_rst1_alphaCorr_lossWeightsMod3'
    ...
    model = Model(dataset_name=dataset_name, model_name=model_name, n_epochs=100, reg_coef=1e-6,
                  ...
                  update_mode=1,
                  init_epochs=30,
                  n_label_resets=1, cut_train_set=False, mod_labels_after_last_reset=True, use_loss_weights=True,
                  log_mask=1 * (1 << 0) +
                           0 * (1 << 1) +
                           0 * (1 << 2) +
                           0 * (1 << 3))
