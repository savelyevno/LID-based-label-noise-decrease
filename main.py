...
if __name__ == '__main__':
    ...
    model_name = '60_paper_ep200_init60'
    ...
    model = Model(dataset_name=dataset_name, model_name=model_name,
                  ...
                  update_mode=1,
                  init_epochs=60,
                  n_label_resets=1, cut_train_set=True, mod_labels_after_last_reset=True, use_loss_weights=False,
                  calc_lid_min_before_init_epoch=False,
                  train_separate_ll=False, separate_ll_class_count=2, separate_ll_count=45, separate_ll_fc_width=64,
                  log_mask=1 * (1 << 0) +
                           0 * (1 << 1) +
                           0 * (1 << 2) +
                           0 * (1 << 3) +
