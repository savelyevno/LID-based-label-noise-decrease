...
def test_mean_and_covariance():
    ...
    for i_epoch in epochs:
        ...
        for i in range(N):
            if keep[i] == 0:
                continue
            x = features[i_epoch, i, :]
            y = y1[i]

            cnts[y] += 1
            means[y] += x

        means = means / cnts.reshape((10, 1))
        # for c in range(10):
        ...
        for i in range(N):
            ...
            diff = (x - means[y]).reshape((-1, 1))
            covariances[y] += np.matmul(diff, diff.T)

        covariance = np.sum(covariances, 0) / np.sum(cnts) + np.eye(d, d) * 1e-9
        covariances = covariances / cnts.reshape((10, 1, 1)) + np.eye(d, d) * 1e-9

        # print(list(covariance[255]))
        inv_covariance = np.linalg.inv(covariance)
        inv_covariances = [np.linalg.inv(it) for it in covariances]
        # for it in covariances:
        #     print(np.linalg.det(it))
        ...
        for i in range(N):
            ...
            diff = (x - means[y]).reshape((-1, 1))
            mah_d = np.matmul(diff.T, np.matmul(inv_covariances[y], diff))
            if mah_d < 0:
                print(mah_d, np.matmul(diff.T, diff))
            mah_d = mah_d ** 0.5
            mah.append(mah_d)
