...
class Model:
    ...
    def _build(self):
        ...
        if self.update_mode == 2 or self.to_log(1):
            ...
            self.class_feature_sums_var = tf.Variable(np.zeros((N_CLASSES, self.total_hidden_width)), dtype=tf.float32)
            ...
            self.class_feature_means_pl = tf.placeholder(tf.float32, (N_CLASSES, self.total_hidden_width))
    ...
    def train(self, train_dataset_type='train'):
        ...
        if self.update_mode == 1 or self.update_mode == 2 or self.update_mode == 3:
            ...
            # similarity of new labels (i.e. with weight 0) compared with true labels without noise but only on clean
            # samples
            ...
            # similarity of new labels (i.e. with weight 0) compared with true labels without noise but only on noised
            # samples
        ...
        if self.to_log(1):
            class_feature_means_per_epoch = np.empty((0, N_CLASSES, self.total_hidden_width))
        if self.to_log(2):
